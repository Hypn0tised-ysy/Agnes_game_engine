...
uint32_t const SCREEN_WIDTH = 960;
uint32_t const SCREEN_HEIGHT = 480;
...
template <typename T> inline void SafeRelease(T **ppInterfaceToRelease) {
...
void InitPipeline() {
  ...
  g_pDev->CreateVertexShader(VS->GetBufferPointer(), VS->GetBufferSize(), NULL,
                             &g_pVS);
  g_pDev->CreatePixelShader(PS->GetBufferPointer(), PS->GetBufferSize(), NULL,
                            &g_pPS);
  ...
  g_pDev->CreateInputLayout(ied, 2, VS->GetBufferPointer(), VS->GetBufferSize(),
                            &g_pLayout);
...
void InitGraphics() {
  ...
  g_pDevcon->Map(g_pVBuffer, NULL, D3D11_MAP_WRITE_DISCARD, NULL,
                 &ms);                               // map the buffer
...
HRESULT CreateGraphicsResources(HWND hWnd) {
  ...
  if (g_pSwapchain == nullptr) {
    ...
    hr = D3D11CreateDeviceAndSwapChain(
        ...
        _countof(FeatureLevels), D3D11_SDK_VERSION, &scd, &g_pSwapchain,
        &g_pDev, &FeatureLevelSupported, &g_pDevcon);

    if (hr == E_INVALIDARG) {
      hr = D3D11CreateDeviceAndSwapChain(
          NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, 0, &FeatureLevelSupported, 1,
          D3D11_SDK_VERSION, &scd, &g_pSwapchain, &g_pDev, NULL, &g_pDevcon);
    }
